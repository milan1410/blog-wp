...
class Blog extends Options {
	...
	/**
	 * Add setting: Show Author's Profile Picture.
	 *
	 * @param WP_Customize_Manager $wp_customize Theme Customizer object.
	 */
	public function set_meta_author_avatar( $wp_customize ) {
		$wp_customize->add_setting(
			'set_meta_author_avatar',
			array(
				'type'              => 'theme_mod',
				'default'           => self::get_meta_author_avatar( true ),
				'transport'         => 'refresh',
				'sanitize_callback' => array( Sanitizer::class, 'sanitize_checkbox' ),
			)
		);
		...
		$wp_customize->add_control(
			...
			array(
				'section'     => 'sec_blog',
				'type'        => 'checkbox',
				'label'       => esc_html__( 'Show Author\'s Profile Picture', 'pressbook' ),
				'description' => esc_html__( 'Replace SVG icon with profile picture on gravatar for post authors.', 'pressbook' ),
			)
		);
	}

	/**
	 * Get setting: Show Author's Profile Picture.
	 *
	 * @param bool $get_default Get default.
	 * @return bool
	 */
	public static function get_meta_author_avatar( $get_default = false ) {
		$default = apply_filters( 'pressbook_default_meta_author_avatar', false );
		if ( $get_default ) {
			return $default;
		}

		return get_theme_mod( 'set_meta_author_avatar', $default );
	}
