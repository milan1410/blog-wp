...
class TemplateTags {
	...
	public static function posted_by() {
		...
		<span class="posted-by byline">
			<?php
			if ( Blog::get_meta_author_avatar() ) {
				echo get_avatar( get_the_author_meta( 'ID' ), 48, '', esc_attr( get_the_author_meta( 'display_name' ) ), array( 'class' => 'svg-icon' ) );
			} else {
				IconsHelper::the_theme_svg( 'user' );
			}
			?>
